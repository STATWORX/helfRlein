...
# create plots ------------------------------------------------------------
l <- igraph::layout_with_fr(g1)
#colrs <- rainbow(length(unique(V(g1)$color)))
colrs <- terrain.colors(length(unique(igraph::V(g1)$color)))

plot(g1,
     edge.arrow.size=.05,
     #vertex.label=NA,
     edge.width = 5*igraph::E(g1)$weight/max(igraph::E(g1)$weight),
     #vertex.shape="none",
     vertex.shape="rectangle",
     vertex.size = (sapply(igraph::V(g1)$label, nchar))*2,
     vertex.size2 = 5,
     ...
     vertex.color = colrs[igraph::V(g1)$color],
     edge.color="steelblue1",
     layout = l)
 legend(x=0,
        unique(igraph::V(g1)$folder), pch=21,
        #col="#777777",
        pt.bg= colrs[unique(igraph::V(g1)$color)],
...
plot.ggnet <- ggnet2(fnet,
  node.size = igraph::V(g1)$size,
  color = igraph::V(g1)$folder,
  #palette = "Blues",
  arrow.size = 12,
  arrow.gap = 0.025,
  label = TRUE,
